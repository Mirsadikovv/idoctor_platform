...
export function getAccessibleRoutes(): AccessibleRoute[] {
	const flatRoutes = flattenRoutes(routers());
	const authStore = useAuthStore();

	const hasRouteAccess = (route: RouteRecordRaw): boolean => {
		return !!route.name && !!authStore.canPage(String(route.name));
	};

	const hasGroupAccess = (groupRoute: RouteRecordRaw): boolean => {
		if (!groupRoute.children) return false;

		return groupRoute.children.some((child) => child.meta?.sidebar && hasRouteAccess(child));
	};
	...
	const result = new Map<string, AccessibleRoute>();

	const addRoute = (route: RouteRecordRaw) => {
		if (!route.name) return;

		const label = route.meta?.sidebar?.label;
		if (!label) return;

		result.set(String(route.name), {
	...
	// одиночные роуты
	sidebarRoutes.forEach(addRoute);

	// группы → дети
	accessibleExpandedRoutes.forEach((group) => {
		group.children?.filter((r) => r.meta?.sidebar && hasRouteAccess(r)).forEach(addRoute);
	});
