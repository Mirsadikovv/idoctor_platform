...
import { UserService, ProblemService, PartService, SupplierService } from "@/service";
import type { User, ProblemType, PartType, SupplierPartialType } from "@/service";
...
export async function searchSuppliers(query: string): Promise<SupplierPartialType[]> {
	const params = buildQuery({
		name: query,
	});

	const supliers = await SupplierService.search(params);
