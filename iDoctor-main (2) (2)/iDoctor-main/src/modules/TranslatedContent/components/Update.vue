...
const { keyWord, fetch } = defineProps<Props>();
...
initalBatch().then((res) => {
	model.value = res;
});

LanguageContentService.getByKey(keyWord).then(
	(res) =>
		(model.value = {
			...
			contents: langStore._languages
				...
				.map((item) => ({
					...
					languageId:
						res.contents.find((i) => i.languageId === item.id)?.languageId || item.id,
					value: res.contents.find((i) => i.languageId === item.id)?.value || "",
					key: res.key,
				})),
		}),
);
			...
			<div class="col-12">
				<div class="row my-row justify-center">
					<div class="col max-w-220px!">
						<Button :loading="loading" class="w-full!" ui-type="danger" v-close-popup>
							{{ $tl("cancel") }}
						</Button>
					</div>
					<div class="col max-w-220px!">
