...
import { PartService, type PartCreateType } from "../service";
...
const partModel = ref<Partial<PartCreateType>>({});

async function save(model: PartCreateType) {
	const response = await PartService.create(model);
		...
		<q-page-container>
			...
			>
				...
				<Form v-model="partModel" :save="save">
					<template #title>
						<Title class="mb-5">{{ $tl("create_part") }}</Title>
					</template>
